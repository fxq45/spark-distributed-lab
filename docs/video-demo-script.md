...
**操作**：在终端或编辑器中打开 `pagerank.py`

```bash
cat src/pagerank.py
```

### 3.1 PageRank 源码讲解（约 1-1.5 分钟）
...
### 5.1 查看 Web UI — Executor 分布（约 1 分钟）
...
## 第六部分：运行 TF-IDF 文本相似度分析（约 1.5-2 分钟）
...
| 3 | 源码讲解（PageRank + TF-IDF） | 2-3 分钟 |
| 4 | 启动集群 + Web UI 验证 | 1-1.5 分钟 |
| 5 | 运行 PageRank + 结果讲解 + Web UI Executor | 2-3 分钟 |
| 6 | 运行 TF-IDF + 结果讲解 | 1.5-2 分钟 |
| 7 | 分布式特性分析 | 30 秒 |
| 8 | 停止集群 + 结尾 | 30 秒 |
| **总计** | | **约 9-10 分钟** |
